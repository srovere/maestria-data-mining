...
A continuación, se presenta el segundo modelo, basado en la superficie total. Encontramos que $\beta_0 = 97815,93$. Por lo explicado para el primer modelo, no tiene mucho sentido interpretar el valor de este parámetro. El valor de la pendiente es $\beta_1 = 1281,07$. Este parámetro es *muy importante* y se puede interpretar como el precio por metro cuadrado de superficie total. Se observa que este valor es muy inferior a los valores que se manejan actualmente para propiedades de Capital Federal. Esto obedece a dos razones:

  * El valor de la ordenada al origen hace que cualquier propiedad parta (de base) de un valor cercano a los USD 100.000. Esto deprime el valor del metro cuadrado llevándolo a un precio mucho menor.
  * También debe tenerse en cuenta que este valor representa el precio por metro cuadrado total, sin importar si esa superficie está cubierta o no. Entonces debe tenerse cuidado en no comparar este valor con los precios que se manejan normalmente, los cuales en general son para metros cuadrados cubiertos.

Ambos parámetros son muy significativos dado que tienen un *p-valor* extremadamente bajo. A continuación, se muestra un gráfico de la recta del ajuste con los valores de los parámetros.
