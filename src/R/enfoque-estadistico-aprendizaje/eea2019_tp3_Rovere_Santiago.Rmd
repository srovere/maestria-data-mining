...
data.rose <- data.frame(Pclass = factor(1, levels = levels(datos_entrenamiento$Pclass)), 
                        Sex = factor("female", levels = levels(datos_entrenamiento$Sex)),
                        Age = 17)
prob.rose <- predict(object = modelo.1, newdata = data.rose, type = "response")

data.jack <- data.frame(Pclass = factor(3, levels = levels(datos_entrenamiento$Pclass)), 
