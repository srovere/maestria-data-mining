...
censo <- sf::st_read(dsn = "data", layer = "censo_2010_caba") %>%
...
centroides.radios.censales <- sf::st_centroid(censo) %>%
  ...
  dplyr::select(ID, barrio_id) %>%
  dplyr::filter(! is.na(barrio_id))
censo <- censo %>%
  dplyr::inner_join(centroides.radios.censales, by = "ID") %>%
  dplyr::select(ID, TOTAL_POB, T_VARON, T_MUJER, T_VIVIENDA, V_PARTICUL, V_COLECTIV, T_HOGAR, H_CON_NBI, H_SIN_NBI, barrio_id)
rm(centroides.radios.censales)
```
...
ggplot2::ggplot() +
  ggplot2::geom_sf(data = establecimientos.por.barrio, colour = NA,
                   mapping = ggplot2::aes(fill = densidad)) +
  ggplot2::labs(x = "", y = "", title = "Establecimientos educativos por barrio",
                subtitle = "Densidad por km²",
                fill = "Densidad por km²") +
  ggplot2::scale_fill_distiller(type = "seq", palette = "YlOrRd") +
  ggplot2::theme_bw() +
  ggplot2::theme(
...
establecimientos.por.barrio <- establecimientos %>%
  ...
  dplyr::group_by(barrio_id) %>%
  dplyr::summarise(cantidad = dplyr::n())
poblacion.por.barrio <- censo %>%
  sf::st_set_geometry(NULL) %>%
  dplyr::group_by(barrio_id) %>%
  dplyr::summarise(habitantes = sum(TOTAL_POB))
habitantes.por.establecimiento.barrio <- barrios %>%
  dplyr::inner_join(establecimientos.por.barrio, by = c("barrio_id")) %>%
  dplyr::inner_join(poblacion.por.barrio, by = c("barrio_id")) %>%
  dplyr::mutate(habitantes_establecimiento = habitantes / cantidad)

ggplot2::ggplot() +
  ggplot2::geom_sf(data = habitantes.por.establecimiento.barrio, colour = NA,
                   mapping = ggplot2::aes(fill = habitantes_establecimiento)) +
  ggplot2::labs(x = "", y = "", title = "Habitantes por establecimientos educativos",
                subtitle = "",
                fill = "Habitantes promedio por establecimiento") +
  ggplot2::scale_fill_distiller(type = "seq", palette = "YlOrRd") +
  ggplot2::theme_bw() +
  ggplot2::theme(
    plot.title = ggplot2::element_text(hjust = 0.5),
...
resumen.barrio <- censo %>%
  sf::st_set_geometry(NULL) %>%
  dplyr::group_by(barrio_id) %>%
  dplyr::summarise(total_hogares = sum(T_HOGAR), total_hogares_nbi = sum(H_CON_NBI)) %>%
...
ggplot2::ggplot() +
  ggplot2::geom_sf(data = porcentaje.nbi.barrio, colour = NA,
