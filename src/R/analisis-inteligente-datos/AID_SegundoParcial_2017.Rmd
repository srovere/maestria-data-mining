...
## c. Selección de la cantidad de clusters
```{r, echo=FALSE, warnings=FALSE, message=FALSE }
metricas <- purrr::map_dfr(
  .x = seq(from = 2, to = nrow(scores)-1),
  .f = function(k) {
    cluster.kmeans.k <- stats::kmeans(x = scores, centers = k, iter.max = 100)
    return (data.frame(k = k, metrica = 'WSS', valor = cluster.kmeans.k$tot.withinss, stringsAsFactors = FALSE) %>%
              dplyr::bind_rows(data.frame(k = k, metrica = 'BSS', valor = cluster.kmeans.k$betweenss, stringsAsFactors = FALSE)))
  }
)
ggplot2::ggplot(data = metricas) +
  ...
  ggplot2::labs(x = "Cantidad de clusters", y = "Valor", col = "Métrica",
                ...
                subtitle = "Comparación de within-cluster sum of squares vs. between-cluster sum of squares") +
  ggplot2::theme_bw() +
  ggplot2::theme(
...
```{r, echo=FALSE, warnings=FALSE, message=TRUE }
modelo.qda <- MASS::qda(minoria ~ ., data = empleados, CV = TRUE)
print("Matriz de contingencia")
table(modelo.qda$class, empleados$minoria)
sprintf("Clasifica correstamente el %.2f%% de los casos", 100 * mean(modelo.qda$class == empleados$minoria))
```
...
## d. Excluimos el último individuo, re-entrenamos y predecimos su clase.
