...
Posteriormente, este conjunto de datos fue introducido en una base de datos MongoDB a fin de poder manejar más cómodamente las consultas. El conjunto de datos inicial es el siguiente:
...
Para el siguiente análisis se obtuvieron los datos del Indice de Precios al Consumidor del Gobierno de la Ciudad de Buenos Aires (IPC GCBA). El IPC GBCA calcula evoluciones de precios para distintas categorías. En este análisis se compararon los valores de las datos relevados contra los valores del IPC GCBA Nivel General [4] e IPC GCBA Alimentos [5]). 

En este primer boxplot se observa que tanto la media como la mediana de las evoluciones de precios calculadas en base a los relevamientos es levemente inferior a los índices de variación del IPC. Esto puede deberse a que tanto en el caso de IPC GCBA - Nivel General como alimentos, la población de productos considerados es sustancialmente diferente a la de los productos relevados en este conjunto de datos. De todos modos, la evolución de precios para el período analizado parece guardar relación con la información publicada por el Gobierno de la Ciudad de Buenos Aires.
...
Además, se desagregó el cálculo de evolución de precios para analizar si existen diferencias de acuerdoa al tipo de sucursarl (supermercados e hipermercados). Tal como se observa en la figura siguente, no existen diferencias notables que muestren una variación diferente de precios entre un tipo de sucursal y otro.
...
Se observa, por ejemplo, que *Supermercados DIA* (que es la unidad de negocios que más veces consiguió el primer puesto en el ranking de comercios más baratos) fue la empresas que más aumentó los precios, mientras que *Hipermercados Carrefour* fue quien menos los aumentó. Esta información es consistente con el gráfico de ranking de precios, ya que *Supermercados DIA* pasa del puesto número 1 al número 3 en dicho ranking y lo inverso le sucede a *Hipermercados Carrefour*.
